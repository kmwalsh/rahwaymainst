...
<section class="dashboard-business-list" id="business-list">
    <section class="standard-box">
        ...
        <h3 class="dashboard-business-name">Edit {{ $business->name }}</h3>
        ...
        <form class="form-wrapper dashboard-business-form" method="POST" action="/edit/{{$business->id}}" add enctype="multipart/form-data">
        
        @csrf

        <fieldset>
            <label for="name"><span class="form-label">Name<span class="form-required">*</span></span>
                <input class="form-input" text-red-600 type="text" name="name" value="{{ old('name', $business->name ) }}">
            </label>
        </fieldset>
        <fieldset>
            <label for="description"><span class="form-label">Description<span class="form-required">*</span></span>
                <textarea maxlength="255" class="js-count-text form-input"  text-red-600 name="description">{{ old('description', $business->description ) }}</textarea>
        ...
        <section class="lg:grid lg:grid-cols-3 lg:gap-10">
            <fieldset class="mb-4">
                <label for="address"><span class="form-label">Address<span class="form-required">*</span></span>
                    <input class="form-input" text-red-600 type="text" name="address" value="{{ old('address', $business->address ) }}">
            ...
            <fieldset class="mb-4">
                <label for="phone"><span class="form-label">Phone<span class="form-required">*</span></span>
                    <input class="form-input" text-red-600 type="phone" name="phone" value="{{ old('phone', $business->phone ) }}">
                </label>
            </fieldset>
            <fieldset class="mb-4">
                <label for="store"><span class="form-label">Online Store, Virtual Office, Facebook Account, Etc.</span>
                    <input class="form-input" type="text" name="store" value="{{ old('store', $business->store ) }}">
                </label>
            </fieldset>
        </section>
        <fieldset class="business-form-hours">
            <label for="hours"><span class="form-label">COVID-19 Special Procedures (Hours, Office Procedures, Take-Out, Virtual Appointments, Etc.)<span class="form-required">*</span></span>
                <textarea maxlength="1000" class="js-count-text form-input business-form-hours-input" name="hours">{{ old('hours', $business->hours ) }}</textarea>
                <span class="form-count-characters js-count-characters">
                    <span class="counter">0</span>/<span class="maxlength">0</span>
                    <span class="counter-error"></span>
                </span>
        ...
        <fieldset>
            ...
            <a href="/home" class="inline-block ml-2">Cancel Editing</a>
        </fieldset>
        </form>
    </section>
